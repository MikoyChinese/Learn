...
from thirdparty.sync_batchnorm import SynchronizedBatchNorm2d


def invert_affine(metas: Union[float, list, tuple], preds):
    # metas, the union type, float, list or tuple.
...
def postprocess(x, anchors, regression, classification,
    ...
    for i in range(x.shape[0]):
        ...
        anchors_nms_idx = batched_nms(transformed_anchors_per, scores_per[:, 0],
                                      classes_, iou_threshold=iou_threshold)
...
class CustomDataParallel(nn.DataParallel):
    """
    Force splitting data to all gpus instead of sending all data to cuda:0
    ...
    def scatter(self, inputs, kwargs):
        # More like scatter and data prep at the same time. The point is we prep
        # the data in such a way that no scatter is necessary, and there's no
        # nedd to shuffle stuff around different GPUs.
        devices = ['cuda:' + str(x) for x in range(self.num_gpus)]
        splits = inputs[0].shape[0] // self.num_gpus

        if splits == 0:
            raise Exception('[Error]: Batch size must be greater than num_gpus.')
