...
class CocoDataset(Dataset):
    ...
    def __init__(self, root_dir, set='train2017', transform=None):
        ...
        self.coco = COCO(os.path.join(self.root_dir,
                                      'annotations',
                                      'instances_',
                                      self.set_name,
                                      '.json'))
...
def collater(data):
    ...
    if max_num_annots > 0:
        ...
        for idx, annot in enumerate(annots):
            if annot.shape[0] > 0:
                annot_padded[idx, :annots.shape[0], :] = annot
    else:
...
class Resizer(object):
    ...
    def __call__(self, sample):
        ...
        if height > width:
            ...
            scale = self.img_szie / width
            resize_width = self.img_szie
            resize_height = int(height * scale)
        image = cv2.resize(image, (resize_width, resize_height),
                           interpolation=cv2.INTER_LINEAR)
        new_image = np.zeros((self.img_szie, self.img_szie, 3))
